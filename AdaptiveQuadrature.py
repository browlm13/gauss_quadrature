...
class AdaptiveQuadrature:
	...
	def __init__(self, low_order_method, high_order_method):
	...
	def sub_divide(self): #, min_h=0.187, tol=None, err=None):#min_h=1e-01): #min_h=0.25): #2):
		...
		if self.tol is not None and err is not None:
			#print(self.tol/err)
			if self.tol/err >= 1e-06:
	...
	def quad(self, a, b):
		...
		#In1 = 0.5*(b-a)*sum([w*fx for w,fx in zip(unit_weights1,fxs1)]) 
		#In2 = 0.5*(b-a)*sum([w*fx for w,fx in zip(unit_weights2,fxs2)])
	...
	def adaptive_quadrature(self, f, a, b, tol=1e-10, maxit=1000, min_h=1e-06, output=False):
